...
// ─────────────────────────────────────────────────────────────
const T = {
  bg: "#07030f", bg1: "#0d0820", bg2: "#120d28", bg3: "#1a1035",
  card: "#150f2a", cardBorder: "#2d1f5e",
  purple: "#7c3aed", purpleLight: "#a855f7",
  green: "#10d98a", red: "#ef4444",
  text: "#e2d9f3", textDim: "#9d8ec4", textMuted: "#5a4a7a",
  bull: "#10d98a", bear: "#ef4444",
  grid: "#1a1040",
...
const CHART_ICONS = { Candlestick:"🕯", "Heikin Ashi":"🕯", Line:"📈", Area:"📊", Bar:"📉", "Hollow Candle":"🕯" };
...
const TOOL_GROUPS = [
  { id:"g_cursor", tools:[
    { id:"cursor",    icon:"↖", label:"Arrow / Cursor",      hotkey:"" },
    { id:"crosshair", icon:"⊕", label:"Crosshair",           hotkey:"" },
  ]},
  { id:"g_lines", divider:true, tools:[
    { id:"trend",    icon:"╱", label:"Trend Line",           hotkey:"Alt+T" },
    { id:"ray",      icon:"→", label:"Ray",                  hotkey:"" },
    { id:"extended", icon:"↔", label:"Extended Line",        hotkey:"" },
    { id:"hline",    icon:"─", label:"Horizontal Line",      hotkey:"Alt+H" },
    { id:"vline",    icon:"│", label:"Vertical Line",        hotkey:"" },
  ]},
  { id:"g_channel", tools:[
    { id:"channel",   icon:"⋮", label:"Parallel Channel",   hotkey:"" },
    { id:"pitchfork", icon:"ψ", label:"Andrews' Pitchfork", hotkey:"" },
  ]},
  { id:"g_fib", tools:[
    { id:"fib_ret", icon:"ƒ", label:"Fib Retracement",      hotkey:"Alt+F" },
    { id:"fib_ext", icon:"ℱ", label:"Fib Extension",        hotkey:"" },
    { id:"fib_fan", icon:"⌂", label:"Fib Fan",              hotkey:"" },
    { id:"fib_tz",  icon:"⏱", label:"Fib Time Zone",        hotkey:"" },
    { id:"fib_arc", icon:"◒", label:"Fib Arc",              hotkey:"" },
  ]},
  { id:"g_gann", tools:[
    { id:"gann_box", icon:"⊞", label:"Gann Box",            hotkey:"" },
    { id:"gann_fan", icon:"◁", label:"Gann Fan",            hotkey:"" },
  ]},
  { id:"g_shapes", divider:true, tools:[
    { id:"rect",     icon:"▭", label:"Rectangle",           hotkey:"" },
    { id:"ellipse",  icon:"○", label:"Ellipse",             hotkey:"" },
    { id:"triangle", icon:"△", label:"Triangle",            hotkey:"" },
    { id:"arrow_up", icon:"↑", label:"Arrow Up",            hotkey:"" },
    { id:"arrow_dn", icon:"↓", label:"Arrow Down",          hotkey:"" },
  ]},
  { id:"g_text", tools:[
    { id:"text",    icon:"T",  label:"Text Label",          hotkey:"Alt+L" },
    { id:"note",    icon:"✎",  label:"Sticky Note",         hotkey:"" },
    { id:"callout", icon:"💬", label:"Callout",             hotkey:"" },
  ]},
  { id:"g_draw", divider:true, tools:[
    { id:"brush",       icon:"✏", label:"Brush",            hotkey:"" },
    { id:"highlighter", icon:"✦", label:"Highlighter",      hotkey:"" },
    { id:"eraser",      icon:"⌫", label:"Eraser",           hotkey:"" },
  ]},
];
...
// ─────────────────────────────────────────────────────────────
const LAYOUTS = {
  single: { label:"Single",       panels:1, cols:1, rows:1 },
  h2:     { label:"Side by Side", panels:2, cols:2, rows:1 },
  v2:     { label:"Stacked",      panels:2, cols:1, rows:2 },
  quad:   { label:"4 Charts",     panels:4, cols:2, rows:2 },
};

// ─────────────────────────────────────────────────────────────
// DEFAULT PANEL CONFIG
// ─────────────────────────────────────────────────────────────
const mkPanel = (sym="NQ", tf="5m") => ({
  sym, tf, chartType:"Candlestick", drawings:[],
  settings:{ bullColor:"#10d98a", bearColor:"#ef4444", showVolume:true, showGrid:true },
});

// ─────────────────────────────────────────────────────────────
// DATA GENERATION — tf-aware
// ─────────────────────────────────────────────────────────────
const dataCache = {};
function genData(sym, tf="5m", count=800) {
  const key = `${sym}_${tf}`;
  if (dataCache[key]) return dataCache[key];
  const { base, vol } = INSTRUMENTS[sym];
  const gap = TF_MS[tf] || 300000;
  const bars = [];
  let price = base;
  const now = Date.now();
...
function fmtTime(ts, tf="5m") {
  const d = new Date(ts);
  if (["1D","1W","1M"].includes(tf))
    return `${d.getMonth()+1}/${d.getDate()}/${String(d.getFullYear()).slice(2)}`;
  if (["4H","2H","1H"].includes(tf))
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      stars.forEach(s => {
        ...
        ctx.globalAlpha=s.a*0.8; ctx.fillStyle=s.col;
...
// CANVAS CHART
// ─────────────────────────────────────────────────────────────
function CandlestickChart({ data, chartType, tf="5m", replayIdx, drawings, activeTool, onAddDrawing, settings={}, isActive=true, onActivate }) {
  ...
  const S = { bullColor:T.bull, bearColor:T.bear, showVolume:true, showGrid:true, ...settings };
  ...
  useEffect(() => {
    ...
    drawChart(canvas, visData, view, mouse, chartType, tf, [...drawings, ...(tmpDraw?[tmpDraw]:[])], S);
  });

  function drawChart(canvas, vd, view, mouse, chartType, tf, allDrawings, S) {
    ...
    if (S.showGrid) {
      ...
      for (let i=0;i<=8;i++) {
        const y=toY(minP+((maxP-minP)*i)/8);
    ...
    if (S.showVolume) {
      ...
      slice.forEach((b,i) => {
        const x=toX(i), bw=Math.max(1,(cW/slice.length)*0.6);
    ...
    if (chartType==="Line"||chartType==="Area") {
      if (chartType==="Area") {
        ...
        ctx.lineTo(toX(slice.length-1),cH); ctx.lineTo(toX(0),cH);
      ...
      slice.forEach((b,i) => {
        const x=toX(i); const col=b.c>=b.o?S.bullColor:S.bearColor;
    ...
    for (let i=0;i<=6;i++) {
      ...
      if (slice[idx]) ctx.fillText(fmtTime(slice[idx].t, tf), PL+(cW*i)/6, cH+20);
    }
    ...
    if (mouse&&mouse.x>PL&&mouse.x<W-PR&&mouse.y>0&&mouse.y<cH) {
      ...
      if (cb) {
        ...
        ctx.fillRect(PL,4,360,24);
        ctx.fillStyle=cb.c>=cb.o?S.bullColor:S.bearColor;
  ...
  function renderDrawing(ctx,d,startOffset,slice,toX,toY,cH,W,PR) {
    if (!d||!d.p1) return;
    const absToRel=(bi)=>bi-startOffset;
    ...
    if (d.tool==="trend"||d.tool==="ray"||d.tool==="extended") {
      ...
      if (!c2){ctx.restore();return;}
      [0.382,0.5,0.618].forEach((lvl,li)=>{
      ...
      for (let i=1;i<=8;i++) {
        const x=c1.x+(c2.x-c1.x)*i/8, y=c1.y+(c2.y-c1.y)*i/8;
      ...
      ctx.beginPath();
      ctx.moveTo(c1.x,c1.y-14*dir);
      ctx.lineTo(c1.x+8,c1.y); ctx.lineTo(c1.x-8,c1.y);
      ctx.closePath(); ctx.fill();
      ctx.lineWidth=2;
      ctx.beginPath(); ctx.moveTo(c1.x,c1.y); ctx.lineTo(c1.x,c1.y+20*dir); ctx.stroke();
  ...
  return (
    <div ref={wrapRef} style={{flex:1,position:"relative",overflow:"hidden",background:T.bg,outline:isActive?"2px solid rgba(124,58,237,0.5)":"2px solid transparent",outlineOffset:"-2px"}} onWheel={onWheel}>
...
// ─────────────────────────────────────────────────────────────
function TVToolbar({ activeTool, onSelect, onClear, drawingCount }) {
  ...
  const clearHover = () => {
    hoverTimer.current = setTimeout(() => setHoveredGroup(null), 120);
  };

  const getGroupTool = (group) => {
    const lastId = groupActive[group.id];
    if (lastId) {
      const t = group.tools.find(t => t.id === lastId);
      if (t) return t;
    }
    return group.tools[0];
  };

  const selectTool = (groupId, toolId) => {
    onSelect(toolId);
    setGroupActive(g => ({...g, [groupId]: toolId}));
    setHoveredGroup(null);
  };

  return (
    <div style={{
      width:46, background:T.bg1, borderRight:`1px solid ${T.cardBorder}`,
      display:"flex", flexDirection:"column", alignItems:"center",
      padding:"8px 0", gap:0, flexShrink:0, zIndex:50, position:"relative"
    }}>
      {TOOL_GROUPS.map((group) => {
        const ct = getGroupTool(group);
        ...
        return (
          <div key={group.id} style={{display:"contents"}}>
            {group.divider && (
              <div style={{width:28,height:1,background:T.cardBorder,margin:"6px auto"}} />
            )}
            ...
            >
              <button
                title={ct.label}
                onClick={()=>selectTool(group.id, ct.id)}
                style={{
                  width:34, height:34, borderRadius:7, border:"none", cursor:"pointer",
                  background:isActive||isHovered?"rgba(124,58,237,0.25)":"transparent",
                  color:isActive?T.purpleLight:isHovered?T.text:T.textMuted,
              ...
              >
                ...
                {hasMultiple && (
                  <span style={{
                    ...
                    fontSize:5, color:isActive?T.purpleLight:T.textMuted,
                    lineHeight:1,
                  }}>▶</span>
                )}
              </button>
              ...
              {isHovered && hasMultiple && (
                <div
                  ...
                  style={{
                    ...
                    borderRadius:10, zIndex:300, minWidth:200,
                    padding:"5px 0", boxShadow:"0 12px 40px rgba(0,0,0,0.65)",
                    overflow:"hidden",
                  }}
                >
                  {group.tools.map(tool => (
                    <button
                      ...
                      style={{
                        ...
                        background:activeTool===tool.id?"rgba(124,58,237,0.18)":"transparent",
                        border:"none", cursor:"pointer",
                        color:activeTool===tool.id?T.purpleLight:T.text,
                        fontSize:13, transition:"background 0.1s",
                      }}
                      onMouseEnter={e=>e.currentTarget.style.background="rgba(124,58,237,0.12)"}
                      onMouseLeave={e=>e.currentTarget.style.background=activeTool===tool.id?"rgba(124,58,237,0.18)":"transparent"}
                    >
                      <span style={{width:18,textAlign:"center",fontSize:15,flexShrink:0}}>{tool.icon}</span>
                      <span style={{flex:1,textAlign:"left"}}>{tool.label}</span>
                      {tool.hotkey && (
                        <span style={{fontSize:10,color:T.textMuted,fontFamily:"monospace",flexShrink:0}}>{tool.hotkey}</span>
                      )}
                    </button>
                  ))}
                </div>
      ...
      {/* Bottom tools */}
      <div style={{marginTop:"auto",display:"flex",flexDirection:"column",alignItems:"center",gap:2,paddingTop:8,borderTop:`1px solid ${T.cardBorder}`,width:"100%"}}>
        {drawingCount > 0 && (
          <button
            ...
            style={{
              width:34,height:34,borderRadius:7,border:"none",cursor:"pointer",
              background:"rgba(239,68,68,0.12)",color:T.bear,fontSize:14,
...
// INSTRUMENT SELECTOR
// ─────────────────────────────────────────────────────────────
function InstrumentSelector({ current, onChange, compact=false }) {
  ...
  return (
    <div ref={ref} style={{position:"relative"}}>
      <button onClick={()=>setOpen(o=>!o)} style={{
        background:open?"rgba(124,58,237,0.2)":T.bg3,
        ...
        <span>{current}</span>
        {!compact && <span style={{fontSize:11,color:T.textMuted,fontWeight:400}}>{INSTRUMENTS[current]?.cat}</span>}
      ...
      {open && (
        <div style={{
          ...
          background:T.card,border:`1px solid ${T.cardBorder}`,
          borderRadius:14,zIndex:500,boxShadow:"0 24px 60px rgba(0,0,0,0.7)",overflow:"hidden",
        }}>
          ...
          <div style={{display:"flex",gap:4,padding:"7px 10px",borderBottom:`1px solid ${T.cardBorder}`,flexWrap:"wrap"}}>
            {CATS.map(c=>(
              <button key={c} onClick={()=>setFilter(c)} style={{
                padding:"2px 8px",borderRadius:5,border:"none",cursor:"pointer",fontSize:10,
                background:filter===c?T.purple:T.bg3,
                color:filter===c?"#fff":T.textDim,fontWeight:filter===c?700:400,
              }}>{c}</button>
            ))}
          </div>
            ...
            {Object.entries(INSTRUMENTS)
              ...
              .map(([k,v])=>(
                <button key={k} onClick={()=>{onChange(k);setOpen(false);setSearch("");}}
                  style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"space-between",padding:"8px 14px",background:k===current?"rgba(124,58,237,0.15)":"transparent",border:"none",cursor:"pointer",color:T.text,borderBottom:`1px solid rgba(45,31,94,0.2)`,transition:"background 0.1s"}}
...
// ─────────────────────────────────────────────────────────────
// PANEL HEADER — per-chart controls
// ─────────────────────────────────────────────────────────────
function PanelHeader({ panel, onUpdate, isActive, onActivate, onOpenSettings, compact=false }) {
  const [chartMenu, setChartMenu] = useState(false);
  const [tfMenu, setTfMenu] = useState(false);
  const ref = useRef(null);

  useEffect(() => {
    const h = (e) => { if (ref.current && !ref.current.contains(e.target)) { setChartMenu(false); setTfMenu(false); } };
  ...
  return (
    <div ref={ref} onClick={onActivate} style={{
      height:36, background:isActive?"rgba(124,58,237,0.1)":T.bg1,
      borderBottom:`1px solid ${T.cardBorder}`,
      display:"flex", alignItems:"center", padding:"0 8px", gap:6,
      flexShrink:0, cursor:"pointer", userSelect:"none",
      borderTop:isActive?`1px solid rgba(124,58,237,0.35)`:`1px solid transparent`,
    }}>
      {/* Instrument */}
      <div onClick={e=>e.stopPropagation()}>
        <InstrumentSelector current={panel.sym} onChange={sym=>onUpdate({sym})} compact />
      </div>

      {/* Timeframe - compact pills */}
      <div style={{display:"flex",gap:1,flexShrink:0}} onClick={e=>e.stopPropagation()}>
        {(compact ? ["1m","5m","15m","1H","4H","1D"] : TF_LIST).map(t=>(
          <button key={t} onClick={()=>onUpdate({tf:t})} style={{
            padding:"2px 5px",borderRadius:4,border:"none",cursor:"pointer",fontSize:9.5,
            background:panel.tf===t?T.purple:"transparent",
            color:panel.tf===t?"#fff":T.textMuted,
            fontWeight:panel.tf===t?700:400,
          }}>{t}</button>
        ))}
        {compact && (
          <div style={{position:"relative"}} onClick={e=>e.stopPropagation()}>
            <button onClick={()=>setTfMenu(o=>!o)} style={{padding:"2px 5px",borderRadius:4,border:"none",cursor:"pointer",fontSize:9.5,background:"transparent",color:T.textMuted}}>…</button>
            {tfMenu && (
              <div style={{position:"absolute",top:"calc(100%+4px)",left:0,background:T.card,border:`1px solid ${T.cardBorder}`,borderRadius:8,zIndex:300,padding:4,display:"flex",flexWrap:"wrap",gap:2,width:160}}>
                {TF_LIST.map(t=>(
                  <button key={t} onClick={()=>{onUpdate({tf:t});setTfMenu(false);}} style={{padding:"3px 7px",borderRadius:4,border:"none",cursor:"pointer",fontSize:10,background:panel.tf===t?T.purple:"transparent",color:panel.tf===t?"#fff":T.textDim}}>{t}</button>
                ))}
              </div>
            )}
          </div>
      ...
      <div style={{width:1,height:18,background:T.cardBorder,margin:"0 2px"}} />

      {/* Chart type */}
      <div style={{position:"relative"}} onClick={e=>e.stopPropagation()}>
        <button onClick={()=>setChartMenu(o=>!o)} title={panel.chartType} style={{
          background:chartMenu?"rgba(124,58,237,0.2)":T.bg3,
          border:`1px solid ${T.cardBorder}`,color:T.textDim,
          padding:"3px 8px",borderRadius:6,cursor:"pointer",fontSize:11,
          display:"flex",alignItems:"center",gap:4,
        }}>
          {CHART_ICONS[panel.chartType]||"📊"}
          <span style={{fontSize:10}}>{panel.chartType}</span>
          <span style={{fontSize:8}}>▾</span>
        </button>
        {chartMenu && (
          <div style={{position:"absolute",top:"calc(100%+4px)",left:0,background:T.card,border:`1px solid ${T.cardBorder}`,borderRadius:10,zIndex:300,overflow:"hidden",minWidth:150}}>
            {CHART_TYPES.map(ct=>(
              <button key={ct} onClick={()=>{onUpdate({chartType:ct});setChartMenu(false);}} style={{
                display:"flex",alignItems:"center",gap:8,width:"100%",textAlign:"left",
                padding:"7px 12px",background:ct===panel.chartType?"rgba(124,58,237,0.15)":"transparent",
                border:"none",color:T.text,cursor:"pointer",fontSize:12,
              }}
              onMouseEnter={e=>e.currentTarget.style.background="rgba(124,58,237,0.1)"}
              onMouseLeave={e=>e.currentTarget.style.background=ct===panel.chartType?"rgba(124,58,237,0.15)":"transparent"}>
                {CHART_ICONS[ct]} {ct}
              </button>
            ))}
          </div>
      ...
      <button
        ...
        onMouseEnter={e=>{e.currentTarget.style.background="rgba(124,58,237,0.15)";e.currentTarget.style.color=T.text;}}
...
// ─────────────────────────────────────────────────────────────
function ChartSettings({ settings, onChange, onClose }) {
  const S = { bullColor:"#10d98a", bearColor:"#ef4444", showVolume:true, showGrid:true, ...settings };

  const Row = ({ label, children }) => (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid rgba(45,31,94,0.3)`}}>
      <span style={{fontSize:12,color:T.textDim}}>{label}</span>
      {children}
    </div>
  );

  const Toggle = ({ checked, onChange: onChg }) => (
    <div onClick={()=>onChg(!checked)} style={{
      width:36,height:20,borderRadius:10,cursor:"pointer",position:"relative",
      background:checked?T.purple:"rgba(45,31,94,0.5)",transition:"background 0.2s",
    }}>
      <div style={{
  ...
  return (
    <div style={{
      position:"absolute",right:48,top:36,zIndex:400,
      background:T.card,border:`1px solid ${T.cardBorder}`,
      borderRadius:14,padding:"14px 16px",width:230,
      boxShadow:"0 16px 48px rgba(0,0,0,0.7)",
    }}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <span style={{fontSize:13,fontWeight:700,color:T.text}}>⚙ Chart Settings</span>
        <button onClick={onClose} style={{background:"transparent",border:"none",color:T.textMuted,cursor:"pointer",fontSize:16}}>✕</button>
      </div>

      <Row label="Bull Candle Color">
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:22,height:22,borderRadius:4,background:S.bullColor,border:`2px solid rgba(255,255,255,0.2)`}} />
          <input type="color" value={S.bullColor} onChange={e=>onChange({...S,bullColor:e.target.value})}
            style={{width:32,height:22,borderRadius:4,border:"none",cursor:"pointer",padding:0,background:"transparent"}} />
        </div>
      </Row>
      <Row label="Bear Candle Color">
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:22,height:22,borderRadius:4,background:S.bearColor,border:`2px solid rgba(255,255,255,0.2)`}} />
          <input type="color" value={S.bearColor} onChange={e=>onChange({...S,bearColor:e.target.value})}
            style={{width:32,height:22,borderRadius:4,border:"none",cursor:"pointer",padding:0,background:"transparent"}} />
        </div>
      </Row>
      <Row label="Show Volume">
        <Toggle checked={S.showVolume} onChange={v=>onChange({...S,showVolume:v})} />
      </Row>
      <Row label="Show Grid">
        <Toggle checked={S.showGrid} onChange={v=>onChange({...S,showGrid:v})} />
      </Row>

      <div style={{marginTop:12,paddingTop:10,borderTop:`1px solid rgba(45,31,94,0.3)`}}>
        <button onClick={()=>onChange({bullColor:"#10d98a",bearColor:"#ef4444",showVolume:true,showGrid:true})}
          style={{width:"100%",padding:"6px",background:"rgba(124,58,237,0.15)",border:`1px solid rgba(124,58,237,0.25)`,color:T.purpleLight,borderRadius:7,cursor:"pointer",fontSize:12}}>
          Reset to Defaults
        </button>
      </div>
    </div>
...
function LayoutPicker({ current, onChange }) {
  ...
  const layoutIcons = {
    single: (
      <svg width="22" height="18" viewBox="0 0 22 18">
        <rect x="1" y="1" width="20" height="16" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/>
      </svg>
    ),
    h2: (
      <svg width="22" height="18" viewBox="0 0 22 18">
        <rect x="1" y="1" width="9" height="16" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/>
        <rect x="12" y="1" width="9" height="16" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/>
      </svg>
    ),
    v2: (
      <svg width="22" height="18" viewBox="0 0 22 18">
        <rect x="1" y="1" width="20" height="7" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/>
        <rect x="1" y="10" width="20" height="7" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/>
      </svg>
    ),
    quad: (
      <svg width="22" height="18" viewBox="0 0 22 18">
        <rect x="1" y="1" width="9" height="7" rx="1.5" fill="none" stroke="currentColor" strokeWidth="1.5"/>
        <rect x="12" y="1" width="9" height="7" rx="1.5" fill="none" stroke="currentColor" strokeWidth="1.5"/>
        <rect x="1" y="10" width="9" height="7" rx="1.5" fill="none" stroke="currentColor" strokeWidth="1.5"/>
        <rect x="12" y="10" width="9" height="7" rx="1.5" fill="none" stroke="currentColor" strokeWidth="1.5"/>
      </svg>
    ),
  };
  ...
  return (
    <div ref={ref} style={{position:"relative"}}>
      <button onClick={()=>setOpen(o=>!o)} title="Chart Layout" style={{
        background:open?"rgba(124,58,237,0.2)":T.bg3,
        border:`1px solid ${T.cardBorder}`,color:T.textDim,
        padding:"4px 8px",borderRadius:8,cursor:"pointer",
        display:"flex",alignItems:"center",gap:5,fontSize:11,
      }}>
        <span style={{color:open?T.purpleLight:T.textDim}}>{layoutIcons[current]}</span>
        <span style={{fontSize:8}}>▾</span>
      </button>
      {open && (
        <div style={{
          ...
          background:T.card,border:`1px solid ${T.cardBorder}`,
          borderRadius:12,zIndex:200,padding:10,
          boxShadow:"0 16px 48px rgba(0,0,0,0.65)",
          display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,width:170,
        }}>
          <div style={{gridColumn:"1/-1",fontSize:10,color:T.textMuted,marginBottom:4,letterSpacing:0.5,textTransform:"uppercase"}}>Layout</div>
          {Object.entries(LAYOUTS).map(([key,lay])=>(
            <button key={key} onClick={()=>{onChange(key);setOpen(false);}} style={{
              display:"flex",flexDirection:"column",alignItems:"center",gap:5,
              padding:"8px 6px",borderRadius:8,border:`1px solid ${key===current?T.purple:T.cardBorder}`,
              background:key===current?"rgba(124,58,237,0.2)":T.bg3,
              cursor:"pointer",color:key===current?T.purpleLight:T.textDim,
              transition:"all 0.15s",
            }}>
              {layoutIcons[key]}
              <span style={{fontSize:9,fontWeight:key===current?700:400}}>{lay.label}</span>
            </button>
          ))}
        </div>
      )}
    ...
    <div style={{width:210,background:T.bg1,borderLeft:`1px solid ${T.cardBorder}`,display:"flex",flexDirection:"column",padding:10,gap:9,overflowY:"auto",flexShrink:0}}>
      ...
      <div style={{display:"flex",gap:4}}>
        {["market","limit","stop"].map(ot=>(
          <button key={ot} onClick={()=>setOrderType(ot)} style={{flex:1,padding:"5px 0",borderRadius:6,border:"none",cursor:"pointer",background:orderType===ot?T.purple:T.bg3,color:orderType===ot?"#fff":T.textDim,fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:0.3}}>{ot}</button>
        ))}
      </div>
      <div>
  ...
  return (
    <div style={{height:54,background:T.bg1,borderTop:`1px solid ${T.cardBorder}`,display:"flex",alignItems:"center",padding:"0 14px",gap:14,flexShrink:0}}>
      ...
      <div style={{display:"flex",gap:4,alignItems:"center"}}>
        ...
        {[0.5,1,2,5,10].map(s=>(
          <button key={s} onClick={()=>onSetSpeed(s)} style={{padding:"3px 8px",borderRadius:5,border:"none",cursor:"pointer",fontSize:10,background:speed===s?T.purple:T.bg3,color:speed===s?"#fff":T.textDim,fontWeight:speed===s?700:400}}>{s}x</button>
        ))}
      </div>
      <div style={{flex:1,display:"flex",alignItems:"center",gap:10}}>
        ...
        <span style={{fontSize:11,color:T.textDim,fontFamily:"monospace",minWidth:120}}>{bar.t?fmtTime(bar.t,"5m"):"—"}</span>
...
function TradingPlatform({ user, onProfile, onLogout }) {
  ...
  const [panels, setPanels] = useState([mkPanel("NQ","5m")]);
  const [activePanel, setActivePanel] = useState(0);
  const [activeTool, setActiveTool] = useState("cursor");
  const [openSettings, setOpenSettings] = useState(null); // panel index with settings open
  ...
  const ap = panels[activePanel] || panels[0];
  const data = useMemo(() => genData(ap.sym, ap.tf), [ap.sym, ap.tf]);

  const updatePanel = useCallback((idx, updates) => {
    setPanels(ps => ps.map((p,i) => i===idx ? {...p,...updates} : p));
  }, []);

  // Adjust panels array when layout changes
  const handleLayoutChange = (newLayout) => {
    const needed = LAYOUTS[newLayout].panels;
    setLayout(newLayout);
    setPanels(prev => {
      if (prev.length >= needed) return prev.slice(0, needed);
      const extras = Array.from({length: needed-prev.length}, (_,i) => {
        const syms=["NQ","ES","GC","CL","BTC/USD","EUR/USD","YM","ETH/USD"];
        return mkPanel(syms[prev.length+i] || "NQ", prev[0]?.tf || "5m");
      });
      return [...prev, ...extras];
    });
    if (activePanel >= needed) setActivePanel(0);
  };

  // Reset replay when active panel's symbol changes
  useEffect(() => {
    setReplayIdx(null);
    setPlaying(false);
  }, [ap.sym]);
  ...
  return (
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:T.bg,color:T.text,fontFamily:"Inter,sans-serif",overflow:"hidden"}}>
      ...
      <div style={{height:50,background:T.bg1,borderBottom:`1px solid ${T.cardBorder}`,display:"flex",alignItems:"center",padding:"0 10px",gap:10,flexShrink:0,zIndex:10}}>
        ...
        {/* Layout Picker */}
        <LayoutPicker current={layout} onChange={handleLayoutChange} />

        <div style={{width:1,height:24,background:T.cardBorder}} />

        {/* Active panel quick info */}
        <span style={{fontSize:12,fontWeight:700,color:T.purpleLight}}>{ap.sym}</span>
        <span style={{fontSize:10,color:T.textMuted,background:T.bg3,padding:"2px 8px",borderRadius:10}}>{ap.tf}</span>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <TVToolbar
          ...
          onClear={()=>updatePanel(activePanel,{drawings:[]})}
          drawingCount={ap.drawings?.length || 0}
        />
        ...
        <div style={{
          ...
          {panels.map((panel, pi) => {
            const panelData = genData(panel.sym, panel.tf);
            const isActive = pi === activePanel;
            const settingsOpen = openSettings === pi;

            return (
              <div key={pi} style={{display:"flex",flexDirection:"column",overflow:"hidden",position:"relative",borderRight:pi%lay.cols!==lay.cols-1?`1px solid ${T.cardBorder}`:"none",borderBottom:Math.floor(pi/lay.cols)!==lay.rows-1?`1px solid ${T.cardBorder}`:"none"}}>
                {/* Per-panel header */}
                <PanelHeader
                  panel={panel}
                  onUpdate={u=>updatePanel(pi,u)}
                  isActive={isActive}
                  onActivate={()=>setActivePanel(pi)}
                  onOpenSettings={()=>setOpenSettings(settingsOpen?null:pi)}
                ...
                <CandlestickChart
                  ...
                  tf={panel.tf}
                  replayIdx={isActive ? replayIdx : null}
                  drawings={panel.drawings}
                  activeTool={isActive ? activeTool : "cursor"}
                  settings={panel.settings}
                  isActive={isActive && panels.length > 1}
                  onActivate={()=>setActivePanel(pi)}
                  onAddDrawing={d=>updatePanel(pi,{drawings:[...panel.drawings,d]})}
                />
                ...
                {settingsOpen && (
                  <ChartSettings
                    settings={panel.settings}
                    onChange={s=>updatePanel(pi,{settings:s})}
        ...
        {/* Order Panel (uses active panel data) */}
        <OrderPanel
          sym={ap.sym} data={data} replayIdx={curIdx}
...
function SplashPage({ onEnter }) {
  ...
  const features = [
    ...
    { icon:"📊", title:"Every Major Market",    desc:"NQ, ES, YM, Gold, Crude, Forex pairs, Crypto, Indices. One platform, every edge." },
    { icon:"✏️", title:"All Drawing Tools",     desc:"Trend lines, Fibonacci, Gann, pitchforks, channels, shapes, text — TradingView-style toolbar." },
    ...
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,#07030f 0%,#0f0520 55%,#07030f 100%)`,color:T.text,fontFamily:"Inter,sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:52,padding:"22px 40px",borderTop:"1px solid rgba(124,58,237,0.1)",borderBottom:"1px solid rgba(124,58,237,0.1)",marginBottom:72}}>
          {[["22+","Markets"],["15","Timeframes"],["28+","Drawing Tools"],["4","Layout Modes"]].map(([n,l])=>(
...
export default function App() {
  ...
  if (sbLoading) return (
    <div style={{height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:T.bg,color:T.text,flexDirection:"column",gap:16}}>
      <div style={{width:32,height:32,borderRadius:8,background:"linear-gradient(135deg,#7c3aed,#10d98a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>⚡</div>
      <div style={{fontSize:13,color:T.textMuted}}>Loading ValorReplay…</div>
    </div>
  );
    ...
    <>
      ...
      {page==="platform" && (
        <TradingPlatform
          ...
          onLogout={async()=>{await supabase.auth.signOut();setUser(null);setPage("splash");}}
        />
      )}
      {page==="profile" && (
        <ProfilePage user={user} trades={trades} balance={balance} onBack={()=>setPage("platform")} />
      )}
      {authMode && (
